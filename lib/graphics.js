    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if ('' !== lines[i].trim()) {
        if (' ' !== lines[i][0] && '\t' !== lines[i][0] && lines[i].toLowerCase().indexOf(' connected ') !== -1) {
          ...
          if (lines[i].indexOf(`normal (normal`) >= 0) {
            currentDisplay.rotation = 0;
          } else if (lines[i].indexOf(`left (normal`) >= 0) {
            currentDisplay.rotation = 270;
          } else if (lines[i].indexOf(`inverted (normal`) >= 0) {
            currentDisplay.rotation = 180;
          } else if (lines[i].indexOf(`right (normal`) >= 0) {
            currentDisplay.rotation = 90;
          }
          
          for (let j = i; j<lines.length; j++) {
            if (lines[j].indexOf('CONNECTOR_ID: ') >= 0) {
              currentDisplay.connectorId = lines[j].split(' ')[1];
              break;
            }
          }
        }
